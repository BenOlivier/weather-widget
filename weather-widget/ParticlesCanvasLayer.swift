...
struct ParticlesCanvasLayer: View {
    /// Parallax offset in points, for example driven by accelerometer tilt.
    /// Positive x moves rain right, positive y moves rain down.
    var parallaxOffset: CGSize = .zero
            ...
            TimelineView(.animation) { timeline in
                Canvas { context, size in
                    ...
                    for drop in drops {
                        ...
                        let parallaxScale = lerp(0.25, 1.0, depth)
                        let parallax = CGSize(width: parallaxOffset.width * parallaxScale, height: parallaxOffset.height * parallaxScale)
                        
                        
                        let x = drop.x01 * size.width + parallax.width
                        let y = drop.y01 * size.height + parallax.height
